...
│   └── .build_done      # ビルド完了フラグ
└── RM_gentoo_build/
    └── gentoo.sh        # 実機（Ubuntu）で直接ビルドする場合のスクリプト
```
...
### 2. USB を挿して `morning.sh` を編集

```bash
lsblk -o NAME,SIZE,TYPE,MOUNTPOINT,LABEL   # USBのデバイス名を確認
```

`morning.sh` 冒頭の1行だけ書き換える:
```bash
USB_DEV="/dev/sdX"   # → /dev/sdb や /dev/sdc に変更
```
